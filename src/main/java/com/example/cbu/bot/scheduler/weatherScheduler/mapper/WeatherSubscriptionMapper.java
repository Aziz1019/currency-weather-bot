package com.example.cbu.bot.scheduler.weatherScheduler.mapper;

import com.example.cbu.entity.real.UserSubscription;
import org.springframework.jdbc.core.RowMapper;

import java.sql.ResultSet;
import java.sql.SQLException;

public class WeatherSubscriptionMapper implements RowMapper<UserSubscription> {
    public static final String ID = "user_id";
    public static final String CITY_NAME = "city_name";
    public static final String WEATHER_TIME = "weather_time";
    public static final String FIRST_NAME = "first_name";

    @Override
    public UserSubscription mapRow(ResultSet rs, int rowNum) throws SQLException {
        UserSubscription responseDTO = new UserSubscription();
        responseDTO.setUserId(rs.getLong(ID));
        responseDTO.setCurrencyTime(rs.getString(WEATHER_TIME));
        responseDTO.setFirstName(rs.getString(FIRST_NAME));
        responseDTO.setCurrencyCode(rs.getString(CITY_NAME));
        return responseDTO;
    }
}
